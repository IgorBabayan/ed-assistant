...
sealed class JournalWatchService : IJournalWatchService
{
	...
	private bool IsCurrentFile(string path) => string.Equals(Path.GetFullPath(path),
		Path.GetFullPath(_currentFile ?? string.Empty), StringComparison.OrdinalIgnoreCase);
	...
	private bool IsNewerLogFile(string path)
	{
		...
		var currentName = Path.GetFileName(_currentFile);
